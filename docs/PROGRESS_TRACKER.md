...
**Công nghệ:** SQLite (Mục đích chạy trên máy tính cá nhân cấu hình 8GB RAM, cấu trúc sẵn sàng đổi lại thành PostgreSQL bằng một dòng URL khi cần thiết)
...
- `heatmap_path` (Varchar, nullable): Đường dẫn lưu ảnh Heatmap (GradCAM/Self-Attention) giải thích vùng mô hình nhìn vào.
...
- **2026-03-15:** Chuyển đổi Database từ PostgreSQL sang SQLite để phù hợp chạy mô phỏng MVP trên máy tính cá nhân macOS 8GB RAM. Thiết kế sử dụng SQLAlchemy đảm bảo dễ dàng migration lại qua Postgres sau này.
- **2026-03-15:** Khởi tạo tài liệu [ARCHITECTURE.md](ARCHITECTURE.md) chốt kiến trúc: Dùng chế độ Mock cho LuX-ViT trên Mac và Model thật trên PC Windows. Bổ sung trường `heatmap_path` để lưu XAI (GradCAM/Self-Attention) biểu diễn vùng AI quan tâm. External LLM API tích hợp Medical RAG (System Prompt).

---

## 3. Checklist Tuần này (Tuần 1 MVP)

- [x] Khởi tạo dự án FastAPI (Cấu trúc thư mục).
- [x] Thiết lập kết nối SQLite qua SQLAlchemy (sẵn sàng chuyển PostgreSQL).
- [x] Định nghĩa các SQLAlchemy Models (Tạo 3 bảng trên).
